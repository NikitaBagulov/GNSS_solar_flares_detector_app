...
def solar_time_params(timestamp):
    y = timestamp.year
    m = timestamp.month
    d = timestamp.day
    hh = timestamp.hour
...
def compute_gsflai_index(dates, time_key):
    if len(dates) < 2:
        return 0.0
    ...
    valid = np.isfinite(vals) & np.isfinite(cos_chi)
    mask = (cos_chi > -0.2) & valid
    ...
    a1 = np.sum((x - x_mean) * (y - y_mean)) / denom
    return float(a1)
