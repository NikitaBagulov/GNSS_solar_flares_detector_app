...
При загрузке `simurg_hdf` в консоли должен появляться размер файла и прогресс загрузки. Ограничения по времени загрузки не задаются. Если соединение оборвётся, частичный `.tmp` файл сохраняется и следующая попытка продолжит докачку.
