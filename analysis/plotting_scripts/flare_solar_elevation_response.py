#!/usr/bin/env python3
"""
Analyze ionospheric response versus solar elevation angle.

The script scans event map files:
    <results_dir>/<event>/maps/map_<product>.h5

For every map point it calculates the solar elevation angle from UTC time,
latitude, and longitude, then aggregates the response in elevation-angle bins.

Supported products by default:
    roti, dtec_2_10, dtec_10_20, dtec_20_60

Outputs:
    response_vs_solar_elevation_points.csv   optional raw point table
    response_vs_solar_elevation_stats.csv    binned statistics
    response_vs_solar_elevation_<product>.png
    response_vs_solar_elevation_all.png

Example:
    python analyze_solar_elevation_response.py \
        --results-dir ./results \
        --output-dir ./outputs/solar_elevation \
...
LOGGER = logging.getLogger("solar_elevation_response")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Calculate ionospheric response versus solar elevation angle."
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/solar_elevation_response"),
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Solar-elevation bin width in degrees.",
    )
    parser.add_argument(
        "--response-mode",
    ...
    parser.add_argument(
        ...
        type=float,
        default=-90.0,
        help="Minimum solar elevation included, degrees.",
    )
    parser.add_argument(
        "--max-elevation",
        type=float,
        default=90.0,
        help="Maximum solar elevation included, degrees.",
    )
    parser.add_argument(
        "--min-count",
        type=int,
...
def solar_declination_and_subsolar_longitude(
    ...
    The equations are adequate for elevation-angle binning and avoid requiring
    astropy or pvlib.
    """
    jd = julian_day(times)
...
def solar_elevation_deg(
    ...
    """Calculate solar elevation for arrays of UTC times and coordinates."""
    ...
    try:
        with h5py.File(path, "r") as file:
            ...
            for time_key, dataset in file["data"].items():
                ...
                elevation = solar_elevation_deg(times, lat, lon)
...
def _parse_event_class(name: str) -> str | None:
    """Extract class letter (C/M/X) from event directory name."""
    m = re.search(r"[_ ]([ABCMX])", name)
    return m.group(1) if m else None


def _filter_last_n_per_class(
    event_dirs: list[Path], n: int
) -> list[Path]:
    """Keep last n event dirs per class (C, M, X), sorted by name."""
...
def aggregate_by_elevation(
    ...
    if min_elevation >= max_elevation:
        raise ValueError("--min-elevation must be below --max-elevation")

    edges = np.arange(
        min_elevation,
        max_elevation + bin_width,
        bin_width,
        dtype=float,
    )
    if edges[-1] < max_elevation:
        edges = np.append(edges, max_elevation)

    selected = points[
        points["solar_elevation_deg"].between(
            min_elevation, max_elevation, inclusive="both"
        )
    ].copy()

    selected["elevation_bin"] = pd.cut(
        selected["solar_elevation_deg"],
        bins=edges,
        include_lowest=True,
        right=False,
    )
    ...
    grouped = (
        selected.groupby(["product", "elevation_bin"], observed=True)["response"]
        .agg(
            count="count",
            mean="mean",
            median="median",
    ...
    return grouped.drop(columns="elevation_bin").sort_values(
        ["product", "elevation_center_deg"]
    )
...
def main() -> None:
    ...
    stats_path = output_dir / "response_vs_solar_elevation_stats.csv"
    stats.to_csv(stats_path, index=False)
    LOGGER.info("Saved %s", stats_path)

    if args.save_points:
        points_path = output_dir / "response_vs_solar_elevation_points.csv"
